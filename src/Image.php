...
class Image
{
    public static function fromString(
        ...
        $textBox = self::imagettfbbox2($fontSize, 0, __DIR__ . "/arial.ttf", $string);
    ...
    public static function imagettfbbox2(
        float $size,
        float $angle,
    ...
    public static function drawCrossLine($image)
    {
        $crossLine = self::calcCrossLinePos(imagesx($image), imagesy($image));
    ...
    public static function calcCrossLinePos(int $areaWidth, int $areaHeight): array
    {
        $from = [
            "x" => 0,
            "y" => rand(0, $areaHeight),
    ...
    public static function toBase64($image): string
    {
        if (is_object($image)) {
            $image = self::getImageResourceData($image);
        }
        $base64 = "data:image/png;base64," . base64_encode($image);
        return $base64;
    }

    public static function getImageResourceData(object $image): string
    {
        ob_start();
        imagepng($image);
        $data = ob_get_clean();
